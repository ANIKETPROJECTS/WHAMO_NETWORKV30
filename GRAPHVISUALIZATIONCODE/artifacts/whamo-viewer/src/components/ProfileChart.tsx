...
import type { SimulationNode } from "@/lib/parser";
...
function FullscreenProfileChart({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col">
      ...
      {/* ── Compact header: title · legend · pipe toggle · zoom · close ── */}
...
export default function ProfileChart({ nodes, units }: ProfileChartProps) {
  ...
  const animate = useCallback((timestamp: number) => {
    ...
    if (elapsed > 50 / speedRef.current) {
      setFrame((f) => {
        const next = f + 1;
        if (next >= MAX_SIM_FRAMES) {
          setPlaying(false);
          return MAX_SIM_FRAMES;
        }
        return next;
      });
  ...
  const simTime = useMemo(() => {
    const maxT = Math.max(
      ...nodes.map((n) => Math.max(n.maxEnergyElevTime, n.minEnergyElevTime))
    );
  ...
  const xTicks = useMemo(() => {
    ...
    return nodes
      .map((_, i) => i)
      .filter((i) => i % step === 0 || i === nodes.length - 1);
  }, [nodes]);
    ...
    <div className="bg-white border border-gray-100 rounded-xl p-6 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-5">
        <div>
          <h3 className="text-base font-bold text-gray-900">
            Profile Graph — Simulation Summary
          </h3>
          <p className="text-xs text-gray-400 mt-0.5">
            Max/Min Energy Elevation envelopes + animated transient HGL
          </p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
            ...
            <div className="flex flex-wrap gap-x-5 gap-y-1 text-xs text-gray-500">
              <span>
                ...
                <strong className="text-blue-600">
                  {stats.peakMaxElev.toFixed(1)} {elevUnit}
                </strong>
              </span>
              <span>
                Min Elev:{" "}
                <strong className="text-red-600">
                  {stats.peakMinElev.toFixed(1)} {elevUnit}
                </strong>
              </span>
            </div>
          )}
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-gray-200 bg-white text-xs text-gray-600 hover:bg-gray-50 hover:border-gray-300 transition-colors font-medium shrink-0"
          >
      ...
      {isStationary && (
        <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 mb-5 text-xs text-amber-700">
          ...
          <span>
            <strong>Steady-state simulation detected.</strong> Max and Min
            Energy Elevations are identical across all nodes — this file has no
            transient variation to animate. The profile below shows the
            steady-state HGL.
          </span>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex flex-wrap items-center gap-5">
          {[
            { color: "#2563eb", label: `Max Energy Elevation (${elevUnit})` },
            { color: "#dc2626", label: `Min Energy Elevation (${elevUnit})` },
            { color: "#16a34a", label: "Current HGL (animated)" },
          ].map((item) => (
          ...
          {hasPipe && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-gray-500">Pipe Centerline</span>
            </div>
          )}
        </div>
        {hasPipe && (
          ...
          >
            {showPipe ? "Hide" : "Show"} Pipe Profile
          </button>
        )}
      </div>
        ...
        <ResponsiveContainer width="100%" height={460}>
          ...
          >
            ...
            <XAxis
              ...
              ticks={xTicks}
              tickFormatter={(idx) => String(nodeLabels[idx] ?? idx)}
              tick={{ fontSize: 10, fill: "#9ca3af" }}
              axisLine={{ stroke: "#e5e7eb" }}
              tickLine={{ stroke: "#e5e7eb" }}
              interval={0}
            ...
            <Tooltip
              content={
                <CustomTooltip nodeLabels={nodeLabels} units={units} />
              }
            />
            {hasPipe && showPipe && (
              <Line
      ...
      <div className="mt-5 bg-gray-50 rounded-xl px-5 py-4">
        <div className="flex items-center gap-4">
          <button
            ...
            disabled={isStationary}
            className={`w-9 h-9 rounded-full text-white flex items-center justify-center transition-colors shadow-sm shrink-0 ${
              isStationary
            ...
            <div className="flex items-center justify-between text-xs text-gray-400">
              <span>
                ...
                <strong className="text-gray-700">
                  {isStationary ? "Steady State" : `${simTime} sec`}
                </strong>
              </span>
              {!isStationary && (
                <span>{Math.round((frame / MAX_SIM_FRAMES) * 100)}%</span>
              )}
            </div>
            <input
              type="range"
              ...
              className={`w-full h-1.5 cursor-pointer ${
                isStationary ? "opacity-30" : "accent-blue-500"
              }`}
            />
          </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <select
              ...
              disabled={isStationary}
              className="text-xs border border-gray-200 rounded px-1.5 py-1 text-gray-600 bg-white disabled:opacity-40"
            >
